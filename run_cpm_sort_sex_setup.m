...
% >> run_cpm_sort_sex_setup({'ravlt','neon'},{'rfMRI_REST1_AP', 'rfMRI_REST1_PA', 'rfMRI_REST2_AP', 'rfMRI_REST2_PA','tfMRI_CARIT', 'tfMRI_FACENAME', 'tfMRI_VISMOTOR'})

% outputs:
%   saves a .mat file for each param in param_list with structs holding all
%   pt info ('pt_struct_allsubjs') and connectivity matrices
%   ('conn_mat_struct_allsubjs') by sex!

%% Implementation
function run_cpm_sort_sex_setup(param_list, scan_type_list)
tic;
...
for param = 1:length(param_list)
    ...
    if strcmp(param_list{param},'ravlt')
        save('ravlt_by_sex_pt_conn_mat.mat', 'pt_struct_by_sex', 'conn_mat_struct_by_sex','-v7.3')
        disp('RAVLT pt info and connectivity matrices saved!')
    end
    if strcmp(param_list{param},'neon')
        save('neon_by_sex_pt_conn_mat.mat', 'pt_struct_by_sex', 'conn_mat_struct_by_sex','-v7.3')
